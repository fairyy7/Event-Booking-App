...
var _listIOSDevices = _interopRequireWildcard(require("./listIOSDevices"));
...
async function runIOS(_, ctx, args) {
  ...
  if (args.listDevices) {
    ...
    const selectedDevice = await (0, _listIOSDevices.promptForDeviceSelection)(availableDevices);
    if (!selectedDevice) {
  ...
  if (!args.device && !args.udid && !args.simulator) {
    ...
    return runOnBootedDevicesSimulators(scheme, xcodeProject, args, bootedDevices, bootedSimulators);
  }
  if (args.device && args.udid) {
